...
public class MainActivity extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener {
    ...
    private static final String PROYECT_ID = "https://kj9gt.app.goo.gl";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        googleApiClient = new GoogleApiClient.Builder(this)
                ...
                .addApi(AppInvite.API) //Añade a Google Api client el API de invite
                .build();
                ...
                .setResultCallback(new ResultCallback<AppInviteInvitationResult>() {
                    ...
                    public void onResult(@NonNull AppInviteInvitationResult appInviteInvitationResult) {
                        if (appInviteInvitationResult.getStatus().isSuccess()){
                            Intent intent = appInviteInvitationResult.getInvitationIntent(); //Obtiene el intent

                            String deepLink = AppInviteReferral.getDeepLink(intent);//http://platzi.com/starwars/
                            String[] strArr = deepLink.split(DEEP_LINK); // Splitt hasta deep link , separa y pone en un vector
                            String id_post = strArr[1]; // ej starwars
    ...
    private String buildDeepLink(String key){
        return PROYECT_ID + "/?link=http://platzi.com/" + key +"/" +"&apn=" + getApplicationContext().getPackageName();
    }

    private void shareDeepLink(String deepLink){
        Intent intent = new Intent(Intent.ACTION_SEND);
        intent.setType("text/plain");
        intent.putExtra(Intent.EXTRA_SUBJECT, "Firebase DeepLink Share"); //Asunto
        intent.putExtra(Intent.EXTRA_TEXT, deepLink); //Pasa el link
